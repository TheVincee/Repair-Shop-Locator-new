...
<body>
    ...
    <!-- Product Accepted Table -->
    <h2>Product Accepted</h2>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Load accepted parts
            function loadAcceptedParts() {
                const xhr = new XMLHttpRequest();
                xhr.open('GET', `status_operations.php?action=listByStatus&status=Accepted`, true);
                xhr.onload = function() {
                    if (this.status == 200) {
                        ...
                        if (!data.error) {
                            ...
                            document.getElementById('acceptedTableBody').innerHTML = html;
                        } else {
                            alert(data.error);
                        }
                    }
